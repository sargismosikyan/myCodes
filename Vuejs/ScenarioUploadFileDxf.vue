    ...
    <div class="ex-file-other ex-file-dxf">
        <b-row class="justify-content-center">
            <b-col class="col-xl-12">
                ...
                <template v-for="(fileData, index) in orderedFiles">
                    <div class="media" :key="index">
                        ...
                        <div class="media-body">
                            ...
                            <div class="mb-4">
                                <template v-for="(level, k) in orderedLevels(fileData.assignedLevels, fileData.groundType)">
                                    <span class="ex-levels-range mr-2" :key="k+ 'span'" v-if="level.singleRangeLevel === 2">
                                        {{ level.levelItemFrom }} {{ $t('global.to') }} {{ level.levelItemTo }}
                                    </span>
                                    <span class="ex-levels-single mr-2" :key="k+ 'span'" v-else >{{ level.levelItem }}</span>
                                </template>
                            </div>
                            <b-collapse :id="'collapse_' + fileData.id" v-model="fileData.showCollapse">
                                <div class="row">
                                    <div class="col-lg-3">
                                        <p class="ex-unit-select-label">{{ $t('scenario_wizard.dxf.ceiling_height') }}</p>
                                        <p class="ex-details-sizes">{{ fileData.dimensionDetails.getData('ceilingHeight') }}</p>
                                    </div>
                                    <div class="col-lg-3">
                                        <p class="ex-unit-select-label">{{ $t('scenario_wizard.dxf.door_height') }}</p>
                                        <p class="ex-details-sizes">{{ fileData.dimensionDetails.getData('doorHeight') }}</p>
                                    </div>
                                    <div class="col-lg-3">
                                        <p class="ex-unit-select-label">{{ $t('scenario_wizard.dxf.window_height') }}</p>
                                        <p class="ex-details-sizes">{{ fileData.dimensionDetails.getData('windowHeight') }}</p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-lg-3">
                                        <p class="ex-unit-select-label">{{ $t('scenario_wizard.dxf.window_sill_height') }}</p>
                                        <p class="ex-details-sizes">{{ fileData.dimensionDetails.getData('windowSillHeight') }}</p>
                                    </div>
                                    <div class="col-lg-3" v-if="fileData.specificLevel">
                                        <p class="ex-unit-select-label">{{ $t('scenario_wizard.dxf.details_level', { level: fileData.groundType == 1 ? 1 : -1 }) }}</p>
                    ...
                    <b-collapse :key="index+ 'col'" :id="'collapseFooter_' + fileData.id" v-model="fileData.showCollapse">
                ...
                <!--First Media End-->
                <button class="ex-cancel-btn mb-5" @click="addDxfFile" >+ {{ $t('scenario_wizard.dxf.add_button') }}</button>
                <hr class="mb-4">
                ...
                <div class="row mb-5">
                    ...
                    <div class="col-lg-4">
                        <label class="ex-unit-select-label">{{ $t('scenario_wizard.dxf.basement_floor_thickness') }}</label>
                        <input type="number" min="0" class="ex-input-floor-roof" name="">
                        <div class="ex-unit-select">
                            <v-select :items="unitItems"
                ...
                <add-dxf-file-modal ref="addDxfFileModal" :add-file="addFile" :update-file="updateFileData"></add-dxf-file-modal>
            </b-col>
        </b-row>
